...
function FavoritesPage() {
  ...
  const likedSongsQueue = useMemo(() => buildPlayableQueue(likedSongs), [likedSongs])
  const recentlyLovedQueue = useMemo(() => buildPlayableQueue(recentlyLoved), [recentlyLoved])
    ...
    <>
      ...
      <AnimatedSection delay={0.08} className="grid gap-3 sm:gap-[18px] xl:grid-cols-[0.95fr_1.45fr]">
        <article className={panelClass}>
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => {
                      void toggleTrack(buildPlayableTrack(track), likedSongsQueue)
                    }}
                    className="grid h-9 w-9 place-items-center rounded-full bg-white text-[var(--route-accent-strong)] shadow-[var(--route-chip-shadow)]"
                  >
                    {isCurrentTrack(buildPlayableTrack(track)) && isPlaying ? <PauseIcon size={15} /> : <PlayIcon size={15} />}
                  </button>
          ...
          <MotionButton
            className="mt-6 rounded-full bg-[var(--route-chip-bg)] px-5 py-3 font-bold text-[var(--route-accent-strong)] shadow-[var(--route-chip-shadow)]"
          >
      ...
      <AnimatedSection delay={0.11} className="grid gap-3 sm:gap-[18px] xl:grid-cols-[0.95fr_1.45fr]">
        ...
        <TiltCard as="article" className="relative rounded-[24px] border border-white/72 bg-linear-to-br from-[color-mix(in_srgb,var(--route-accent-soft)_76%,white)] via-[color-mix(in_srgb,var(--route-accent)_56%,white)] to-[color-mix(in_srgb,var(--route-accent-strong)_46%,white)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.95),var(--route-chip-shadow)] sm:rounded-[28px] sm:p-[24px]">
          <div className="grid items-center gap-4 sm:gap-6 lg:grid-cols-[0.95fr_1.05fr]">
            <div>
              ...
              <button
                type="button"
                onClick={() => {
                  const firstTrack = likedSongsQueue[0]
                  if (firstTrack) {
                    void playTrack(firstTrack, likedSongsQueue)
                  }
                }}
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                    void toggleTrack(buildPlayableTrack(track), recentlyLovedQueue)
                  }}
                  className="grid h-9 w-9 place-items-center rounded-full bg-white text-[var(--route-accent-strong)] shadow-[var(--route-chip-shadow)]"
                >
                  {isCurrentTrack(buildPlayableTrack(track)) && isPlaying ? <PauseIcon size={15} /> : <PlayIcon size={15} />}
                </button>
                <span className="grid h-9 w-9 place-items-center rounded-full bg-[var(--route-chip-bg)] text-[var(--route-accent-strong)] shadow-[var(--route-chip-shadow)]">
                  <HeartIcon size={17} />
                </span>
              </div>
            </motion.div>
