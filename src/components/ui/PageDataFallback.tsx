...
type PageDataFallbackProps = {
  title: string
}

function PageDataFallback({ title }: PageDataFallbackProps) {
  return (
    <motion.section
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="grid gap-4 rounded-[28px] border border-white/72 bg-linear-to-b from-[rgba(255,252,250,0.96)] to-[rgba(255,245,240,0.94)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.95),0_18px_35px_rgba(195,180,216,0.18)] sm:p-6"
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold tracking-[0.22em] text-[#9a8cb7] uppercase">Mock API</p>
          <h2 className="mt-1 text-[1.2rem] font-extrabold text-[#30294f]">{title}</h2>
        </div>
        <span className="rounded-full bg-[#efe6ff] px-3 py-2 text-sm font-bold text-[#8f6aea]">Loading...</span>
      </div>
...
export default PageDataFallback
